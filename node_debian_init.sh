...
APP_PATH="/usr/local/var/lib/node/100km-results"
DAEMON_ARGS="$APP_PATH/app.js"              # path to your node.js server/app
                                            # NB: don't use ~/ in path

DESC="node.js http server"                  # whatever fancy description you like

NODEUSER=node:node                          # USER who OWNS the daemon process (no matter whoever runs the init script)
                                            # user:group (if no group is specified, the primary GID for that user is used)

LOCAL_VAR_RUN=/usr/local/var/run/node       # in case the init script is run by non-root user, you need to
...
do_start()
{
	...
	start-stop-daemon --start --quiet --chdir $APP_PATH --chuid $NODEUSER --make-pidfile --pidfile $PIDFILE --background --exec $DAEMON -- \
		$DAEMON_ARGS \
...
do_stop()
{
	...
	# sleep for some time.
	start-stop-daemon --stop --quiet --oknodo --retry=0/3/KILL/5 --pidfile $PIDFILE --chdir $APP_PATH --chuid $NODEUSER --exec $DAEMON -- $DAEMON_ARGS
	[ "$?" = 2 ] && return 2
